...
GLUSuint initWaterTexture(GLUSfloat waterPlaneLength)
{
	...
	glusFileLoadText("../Example15_ES/shader/WaterTexture.vert.glsl", &vertexSource);
	glusFileLoadText("../Example15_ES/shader/WaterTexture.frag.glsl", &fragmentSource);

	glusProgramBuildFromSource(&g_programWaterTexture, (const GLUSchar**) &vertexSource.text, (const GLUSchar**) &fragmentSource.text);
