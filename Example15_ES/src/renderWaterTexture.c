...
GLUSuint initWaterTexture(GLUSfloat waterPlaneLength)
{
	...
	glusLoadTextFile("../Example15_ES/shader/WaterTexture.vert.glsl", &vertexSource);
	glusLoadTextFile("../Example15_ES/shader/WaterTexture.frag.glsl", &fragmentSource);

	glusBuildProgramFromSource(&g_programWaterTexture, (const GLUSchar**) &vertexSource.text, (const GLUSchar**) &fragmentSource.text);
